...
func (np *nodepoolInfo) surgeCount() int32 {
	...
	if strings.HasSuffix(s, "%") {
		...
		n := int32(math.Ceil(float64(np.nodeCount) * pct / 100.0))
		if n < 1 {
			n = 1
		}
		return n
...
func generateClusterScripts(
	...
	genCPScript := func(step, fromMinor, toMinor int) bool {
		fromVer := getLatestPatch(fromMinor)
		toVer := getLatestPatch(toMinor)
		if toVer == "" {
		...
		fname := filepath.Join(outDir,
			fmt.Sprintf("%d_upgrade_control_plane_%s_to_%s.sh", step, fromVer, toVer))
		if writeErr := writeScript(fname, cpTpl, cpScriptData{
			Step:          step,
		...
		fmt.Printf("  [+] Created: %s  (CP %s → %s)\n", filepath.Base(fname), fromVer, toVer)
		return true
	}
	...
	genNPScript := func(step int, np *nodepoolInfo, fromMinor, toMinor int) bool {
		...
		fname := filepath.Join(outDir,
			fmt.Sprintf("%d_upgrade_%s_%s_to_%s.sh", step, np.name, fromVer, toVer))
		if writeErr := writeScript(fname, npTpl, npScriptData{
			Step:          step,
			ClusterName:   clusterName,
			ResourceGroup: rg,
		...
		fmt.Printf("  [+] Created: %s  (NP %s: %s → %s)\n",
			filepath.Base(fname), np.name, fromVer, toVer)
		return true
	}
	...
	for !abort && (currentCP < targetVersion.minor || minNP < targetVersion.minor) {
		// Phase 1
		maxCP := minNP + 2
		if maxCP > targetVersion.minor {
			maxCP = targetVersion.minor
		}
		for !abort && currentCP < maxCP {
		...
		minNP = getMinNPMinor()
		if currentCP >= targetVersion.minor && minNP >= targetVersion.minor {
			break
		}
	}
...
func writeScript(path string, tpl *template.Template, data interface{}) error {
